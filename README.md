...
**412 symbols + 121 document commands**, a package reference, and ready-made templates,
...
- **Four-mode palette** — a side panel (Activity Bar) with a
  ...
  - **Symbols** — 412 symbols **typeset with KaTeX** in an organized grid: a **RECENT**
...
- **Smart insertion** — empty `{}` become snippet tab stops (`\frac{}{}` → cursor in the
  first slot); structural commands insert **rich snippets** (e.g. `\begin{align}` inserts
  an aligned two-row skeleton with placeholders you Tab through).
